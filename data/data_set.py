...
def TrainTransform(img,boxes,labels):
    ...
    Args:
        img (np.ndarray[int32]) : [h,w,c] with `RGB`, and each piexl in [0,255]
        boxes (np.ndarray[int32]): [n,4] with `xyxy`
        labels (np.ndarray[int32]): [n], the labels of the box
    Return:
        img (np.ndarray[int32]): [c,s',s'] with `BGR`, and each pixel in [-128,127]
        boxes (np.ndarray[int32]): [n',4] with `xyxy`
        labels (np.ndarray[int32]): [n'], the labels of box after transforms
    """
...
class TrainDataset(Dataset):
    ...
    def __getitem__(self,idx):
        ...
        ori_img= self.sdb._get_image(idx) # [c,h,w] 
